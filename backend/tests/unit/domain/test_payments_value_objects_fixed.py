"""Fixed comprehensive tests for payments domain value objects."""
...
class TestPaymentMethod:
    ...
    def test_payment_method_valid_recognized_methods(self):
        """Test various valid recognized payment methods."""
        valid_methods = ["venmo", "zelle", "paypal", "cash", "check"]

        for method_name in valid_methods:
            method = PaymentMethod(method_name)
            assert method.value == method_name

    def test_payment_method_custom_valid_length(self):
        """Test custom payment methods with valid length."""
        method = PaymentMethod("Custom Payment Method")
        assert method.value == "Custom Payment Method"

    def test_payment_method_too_long(self):
        """Test payment method exceeding 50 characters."""
        with pytest.raises(ValueError, match="Payment method must be 50 characters or less"):
            PaymentMethod("A" * 51)

    def test_payment_method_short_unrecognized(self):
        """Test short unrecognized payment method."""
        with pytest.raises(ValueError, match="Payment method must be at least 3 characters"):
            PaymentMethod("XY")

    def test_payment_method_electronic_check(self):
        """Test electronic payment method detection."""
        venmo = PaymentMethod("venmo")
        assert venmo.is_electronic() is True
        assert venmo.is_cash_based() is False

        cash = PaymentMethod("cash")
        assert cash.is_electronic() is False
        assert cash.is_cash_based() is True

    def test_payment_method_case_sensitivity(self):
        """Test payment method case handling."""
        method_upper = PaymentMethod("VENMO")
        method_lower = PaymentMethod("venmo")

        # Both should be valid but retain their case
        assert method_upper.value == "VENMO"
        assert method_lower.value == "venmo"

        # Both should be recognized as electronic
        assert method_upper.is_electronic() is True
        assert method_lower.is_electronic() is True
...
class TestPaymentReference:
    ...
    def test_payment_reference_length_limit(self):
        ...
        # Too long
        with pytest.raises(ValueError, match="Payment reference must be 100 characters or less"):
...
class TestPaymentNotes:
    ...
    def test_payment_notes_whitespace_normalization(self):
        """Test payment notes whitespace normalization."""
        notes = PaymentNotes("  Payment   details  with   spaces  ")
        assert notes.value == "Payment details with spaces"
    ...
    def test_payment_notes_empty_string_allowed(self):
        """Test payment notes allows empty string after normalization."""
        notes = PaymentNotes("   ")
        assert notes.value == ""
    ...
    def test_payment_notes_length_limit(self):
        ...
        # Too long
        with pytest.raises(ValueError, match="Payment notes must be 500 characters or less"):
            PaymentNotes("A" * 501)

    def test_payment_notes_multiline_normalization(self):
        """Test payment notes with multiline text normalization."""
        multiline_notes = "Line 1\nLine 2\n\nLine 3"
        notes = PaymentNotes(multiline_notes)
        # Should normalize multiple whitespaces/newlines to single spaces
        assert notes.value == "Line 1 Line 2 Line 3"

    def test_payment_notes_is_empty_method(self):
        """Test is_empty method."""
        empty_notes = PaymentNotes("")
        assert empty_notes.is_empty() is True

        content_notes = PaymentNotes("Some content")
        assert content_notes.is_empty() is False
...
class TestBalanceStatus:
    ...
    def test_balance_status_empty_string(self):
        """Test balance status with empty string."""
        with pytest.raises(ValueError, match="Balance status must be one of"):
            BalanceStatus("")
    ...
    def test_balance_status_case_sensitive(self):
        """Test balance status is case sensitive."""
        with pytest.raises(ValueError, match="Balance status must be one of"):
            BalanceStatus("SETTLED")  # uppercase not allowed

    def test_balance_status_methods(self):
        """Test balance status utility methods."""
    ...
    def test_balance_status_immutable(self):
        ...
        with pytest.raises(AttributeError):
            status.value = "owes_money"

    def test_balance_status_equality(self):
        """Test balance status equality."""
        status1 = BalanceStatus("settled")
        status2 = BalanceStatus("settled")
        status3 = BalanceStatus("owes_money")
