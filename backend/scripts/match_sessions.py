#!/usr/bin/env python3
"""
Try to match local game numbers to PokerNow external IDs by analyzing player rosters and totals.
"""
...
def calculate_session_signature(session_data: Dict[str, Tuple[Decimal, Decimal]]) -> Tuple[str, Decimal, int]:
    """Calculate a signature for a session: (player_names_sorted, total_buy_ins, player_count)"""
    player_names = tuple(sorted(session_data.keys()))
    total_buy_ins = sum(buy_in for buy_in, _ in session_data.values())
    player_count = len(session_data)
    
    return (player_names, total_buy_ins, player_count)

def find_session_matches(local_data: Dict, db_data: Dict) -> List[Tuple[int, str, float]]:
    """Try to match local games to DB sessions using various heuristics."""
    matches = []
    
    # Create signatures for all sessions
    local_signatures = {}
    db_signatures = {}
    
    for local_game, local_session in local_data.items():
        local_signatures[local_game] = calculate_session_signature(local_session)
    
    for external_id, db_session in db_data.items():
        db_signatures[external_id] = calculate_session_signature(db_session)
    
    # Try exact matches first
    print("=== LOOKING FOR EXACT MATCHES ===")
    for local_game, local_sig in local_signatures.items():
        for external_id, db_sig in db_signatures.items():
            if local_sig == db_sig:
                matches.append((local_game, external_id, 1.0))  # 100% match
                print(f"EXACT MATCH: Local Game {local_game} = DB {external_id}")
                print(f"  Players: {', '.join(local_sig[0])}")
                print(f"  Total Buy-ins: ${local_sig[1]}")
                break
    
    # Remove matched sessions
    matched_local = {match[0] for match in matches}
    matched_db = {match[1] for match in matches}
    
    # Try partial matches (same player count and similar buy-ins)
    print(f"\n=== LOOKING FOR PARTIAL MATCHES ===")
    for local_game, local_session in local_data.items():
        if local_game in matched_local:
            continue
            
        local_sig = local_signatures[local_game]
        local_players, local_total, local_count = local_sig
        
        best_match = None
        best_score = 0
        
        for external_id, db_session in db_data.items():
            if external_id in matched_db:
                continue
                
            db_sig = db_signatures[external_id]
            db_players, db_total, db_count = db_sig
            
            # Must have same player count
            ...
            # Player name overlap
            common_players = len(set(local_players) & set(db_players))
            player_overlap = common_players / local_count
            score += player_overlap * 0.6  # 60% weight for player overlap
            
            # Buy-in total similarity
            if db_total > 0:
                buy_in_similarity = 1 - abs(local_total - db_total) / max(local_total, db_total)
                score += float(buy_in_similarity) * 0.4  # 40% weight for buy-in similarity
            
            if score > best_score and score > 0.3:  # At least 30% similarity
                best_score = score
                best_match = (external_id, db_session)
        
        if best_match:
            external_id, db_session = best_match
            matches.append((local_game, external_id, best_score))
            matched_local.add(local_game)
            matched_db.add(external_id)
            
            print(f"PARTIAL MATCH ({best_score:.1%}): Local Game {local_game} = DB {external_id}")
            local_players = set(local_session.keys())
            db_players = set(db_session.keys())
            common = local_players & db_players
            local_only = local_players - db_players
            db_only = db_players - local_players
            
            print(f"  Common players: {', '.join(sorted(common))}")
            if local_only:
                print(f"  Local only: {', '.join(sorted(local_only))}")
            if db_only:
                print(f"  DB only: {', '.join(sorted(db_only))}")
    
    # Show unmatched sessions
    unmatched_local = set(local_data.keys()) - matched_local
    unmatched_db = set(db_data.keys()) - matched_db
    
    if unmatched_local:
        print(f"\n=== UNMATCHED LOCAL GAMES ({len(unmatched_local)}) ===")
        for game_num in sorted(unmatched_local):
            session = local_data[game_num]
            players = ', '.join(sorted(session.keys()))
            total_buy_ins = sum(buy_in for buy_in, _ in session.values())
            print(f"  Game {game_num}: {players} (${total_buy_ins})")
    
    if unmatched_db:
        print(f"\n=== UNMATCHED DB SESSIONS ({len(unmatched_db)}) ===")
        for external_id in sorted(unmatched_db):
            session = db_data[external_id]
            players = ', '.join(sorted(session.keys()))
            total_buy_ins = sum(buy_in for buy_in, _ in session.values())
            print(f"  {external_id}: {players} (${total_buy_ins})")
    
    return sorted(matches)

def main():
    if len(sys.argv) != 2:
        print("Usage: python match_sessions.py <public_code>")
        sys.exit(1)
...
84    Trevor    150.00    50.00"""
    ...
    print(f"\nLocal games: {len(local_data)}")
    print(f"DB sessions: {len(db_data)}")
    
    print(f"\nTrying to match sessions...")
    matches = find_session_matches(local_data, db_data)
    
    if matches:
        print(f"\n=== SUMMARY OF MATCHES ===")
        for local_game, external_id, score in matches:
            print(f"Local Game {local_game} → DB {external_id} ({score:.1%})")
