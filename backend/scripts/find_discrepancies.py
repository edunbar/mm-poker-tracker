#!/usr/bin/env python3
"""
Find specific sessions/games where discrepancies occur between local data and database.
"""
...
def get_local_player_sessions():
    """Parse local data and return player sessions by game."""
...
84    Trevor    150.00    50.00"""
    ...
    # Parse local data by game and player
    local_sessions = defaultdict(dict)  # local_sessions[game_number][player] = {'buy_in': x, 'cash_out': y}
    ...
    for line in lines:
        ...
        if len(parts) >= 4:
            ...
            local_sessions[game_number][player_name] = {
                'buy_in': buy_in,
                'cash_out': cash_out,
                'net': cash_out - buy_in
            }
...
def get_db_player_sessions(public_code: str):
    """Get player sessions from database."""
    with SessionLocal() as db:
        SQL = text('''
            SELECT
                s.game_number,
                p.display_name AS player,
                sps.buy_in_sum / 100.0 AS buy_in,
                sps.cash_out_sum / 100.0 AS cash_out,
                sps.in_game / 100.0 AS in_game,
                (sps.cash_out_sum + sps.in_game - sps.buy_in_sum) / 100.0 AS net
            ...
            ORDER BY s.game_number, p.display_name;
        ''')
        
        result = db.execute(SQL, {"public_code": public_code}).mappings().all()
        
        # Organize by game number and player
        db_sessions = defaultdict(dict)
        for row in result:
            game_number = row['game_number']
            player = row['player']
            db_sessions[game_number][player] = {
                'buy_in': Decimal(str(row['buy_in'])),
                'cash_out': Decimal(str(row['cash_out'])),
                'in_game': Decimal(str(row['in_game'])),
                'net': Decimal(str(row['net']))
            }
        
        return db_sessions

def compare_sessions(public_code: str):
    """Compare local and database sessions to find discrepancies."""
    print("=== COMPARING LOCAL VS DATABASE SESSIONS ===")
    
    local_sessions = get_local_player_sessions()
    db_sessions = get_db_player_sessions(public_code)
    
    print(f"Local sessions: Games 1-{max(local_sessions.keys())} ({len(local_sessions)} games)")
    print(f"Database sessions: Games {sorted(db_sessions.keys())} ({len(db_sessions)} games)")
    
    # Find discrepant players
    discrepant_players = ['Eric', 'Remy', 'Trevor']
    
    for player in discrepant_players:
        print(f"\n=== ANALYZING {player.upper()} ===")
        
        # Get all local sessions for this player
        local_player_sessions = []
        for game_num, players in local_sessions.items():
            if player in players:
                local_player_sessions.append((game_num, players[player]))
        
        # Get all DB sessions for this player
        db_player_sessions = []
        for game_num, players in db_sessions.items():
            if player in players:
                db_player_sessions.append((game_num, players[player]))
        
        print(f"Local: {len(local_player_sessions)} sessions")
        print(f"Database: {len(db_player_sessions)} sessions")
        
        # Calculate totals
        local_total_buy_in = sum(session[1]['buy_in'] for session in local_player_sessions)
        local_total_cash_out = sum(session[1]['cash_out'] for session in local_player_sessions)
        local_total_net = local_total_cash_out - local_total_buy_in
        
        db_total_buy_in = sum(session[1]['buy_in'] for session in db_player_sessions)
        db_total_cash_out = sum(session[1]['cash_out'] for session in db_player_sessions)
        db_total_net = sum(session[1]['net'] for session in db_player_sessions)
        
        print(f"Local totals: ${local_total_buy_in} -> ${local_total_cash_out} (Net: ${local_total_net})")
        print(f"DB totals: ${db_total_buy_in} -> ${db_total_cash_out} (Net: ${db_total_net})")
        
        # Find games present in local but missing in DB
        local_games = {session[0] for session in local_player_sessions}
        db_games = {session[0] for session in db_player_sessions}
        
        missing_in_db = local_games - db_games
        extra_in_db = db_games - local_games
        
        if missing_in_db:
            print(f"Games in local but NOT in database: {sorted(missing_in_db)}")
            missing_net = sum(local_sessions[game][player]['net'] for game in missing_in_db)
            print(f"Net from missing games: ${missing_net}")
        
        if extra_in_db:
            print(f"Games in database but NOT in local: {sorted(extra_in_db)}")
        
        # Find games with different values
        common_games = local_games & db_games
        discrepant_games = []
        
        for game in common_games:
            local_net = local_sessions[game][player]['net']
            db_net = db_sessions[game][player]['net']
            
            if abs(local_net - db_net) > Decimal('0.01'):
                discrepant_games.append((game, local_net, db_net))
        
        if discrepant_games:
            print(f"Games with different values:")
            for game, local_net, db_net in discrepant_games:
                print(f"  Game {game}: Local ${local_net} vs DB ${db_net}")

def main():
    if len(sys.argv) != 2:
        print("Usage: python find_discrepancies.py <public_code>")
        sys.exit(1)
    
    public_code = sys.argv[1]
    compare_sessions(public_code)
